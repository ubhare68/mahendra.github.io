...
class cmb_Meta_Box {
	...
	public static function autoload_helpers( $class_name ) {
		...
		$dir = get_template_directory() . '/core/admin/metaboxes';
			...
			if ( ! is_admin() ) {
		   	...
		   	wp_register_script( 'wp-color-picker', admin_url( 'js/color-picker.min.js' ), array( 'iris' ), self::CMB_VERSION );
				wp_localize_script( 'wp-color-picker', 'wpColorPickerL10n', array(
					'clear' => __( 'Clear', 'themestudio' ),
					'defaultString' => __( 'Default', 'themestudio' ),
					'pick' => __( 'Select Color', 'themestudio' ),
					'current' => __( 'Current Color', 'themestudio' ),
				) );
			}
		} else {

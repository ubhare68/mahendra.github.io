  ...
  <?php if ( get_option( 'woocommerce_review_rating_verification_required' ) === 'no' || wc_customer_bought_product( '', get_current_user_id(), $product->id ) ) : ?>
    ...
    <div id="review_form_wrapper">
      <div id="review_form">
        ...
        $comment_form = array(
          'title_reply'          => have_comments() ? __( 'Add a Review', '__x__' ) : __( 'Be the First to Review', '__x__' ) . ' &ldquo;' . get_the_title() . '&rdquo;',
        ...
        $comment_form['comment_field'] .= '<p class="comment-form-comment"><label for="comment">' . __( 'Your Review', '__x__' ) . '</label><textarea id="comment" name="comment" cols="45" rows="8" aria-required="true"></textarea>' . wp_nonce_field( 'woocommerce-comment_rating', '_wpnonce', true, false ) . '</p>';

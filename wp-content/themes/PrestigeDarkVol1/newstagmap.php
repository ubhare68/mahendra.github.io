...
*      newstagmap.php
* Brief:       
*      Theme news tag map page template code
* Author:      
*      DigitalCavalry
* Author URI:
       ...
       <?php 
        ...
        for($i = 1; $i <= $TAGS_MAP_COLUMNS_COUNT; $i++)
        {
            ...
            while($j < $count)
            {
                ...
                $out  = '<li>';                            
                $out .= '<a class="link-tip-top" title="'.$data[$j]->count.' '.($data[$j]->count > 1 ? 'topics' : 'topic').'" href="'.get_term_link($data[$j]->slug, 'news_tag').'">'.$data[$j]->name.' ('.$data[$j]->count.')</a>';
                $out .= ($j < $count - 1) ? ',' : '';
                $out .= '</li>';
                echo $out;                                                      
